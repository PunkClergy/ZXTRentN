	...
	<view class="container" :style="{ height: `${safeScreenHeight}px` }" @touchmove.prevent="handleTouchMove">
		...
		<view class="content-container" :style="{
			...
			<view class="main-contentArea-container">
				<!-- 左侧导航 -->
				<scroll-view scroll-y class="left-nav" style="height: 100%;">
				...
				<scroll-view scroll-y class="right-content" :scroll-top="scrollTop" @scroll="onScroll"
					...
					<view v-for="(item, index) in main_coverage" :key="index" class="right-category"
						...
						<view style="display: flex; flex-wrap: wrap; justify-content: flex-start; gap: 5px;">
							<view v-for="(citem, cindex) in item.children" :key="cindex" class="child-item"
								style="width: calc((100% - 10px) / 3); box-sizing: border-box;">
								<view
									style="display: flex;flex-direction: column;justify-content: center;align-items: center;">
									<image :src="`${baseLink}/img/${citem.icon}`" style="width: 60px; height: 60px;" />
									<text style="font-size: 14px;">{{citem.name}}</text>
								</view>
							</view>
						</view>
		...
		<view style="height: 90px;">ssss</view>
	</view>
</template>
	...
	export default {
		data() {
			return {
				...
				activeIndex: 0,
				scrollTop: 0,
				categoryTopPositions: [] // 每个分类在右侧内容中的 top 值
			}
		},
		...
		methods: {
			switchCategory(index) {
				console.log((this.categoryTopPositions))
				this.activeIndex = index;
				this.scrollTop = this.categoryTopPositions[index] || 0;
			},
			calculateCategoryPositions() {
				console.log((this))
				const query = wx.createSelectorQuery ? wx.createSelectorQuery() : my.createSelectorQuery();

				this.categoryTopPositions = [];

				this.main_coverage.forEach((_, index) => {
					query.select('#category-' + index).boundingClientRect(res => {
						if (res) {
							this.categoryTopPositions[index] = res.top;
						}
					}).exec();
				});
			},
			onScroll(e) {
				...
				for (let i = 0; i < this.categoryTopPositions.length; i++) {
					if (scrollTop < this.categoryTopPositions[i]) {
						this.activeIndex = i - 1 >= 0 ? i - 1 : 0;
						return;
					}
				}

				this.activeIndex = this.categoryTopPositions.length - 1;
			},
			handleTouchMove(e) {
				// 阻止默认滚动行为
				return false;
