<template>
  ...
  <view class="container" :style="{ height: `${safeScreenHeight}px` }">
      ...
      <view class="nav-bar" :style="{ height: navBarHeight + 'px' }">
        <view class="title-container">
          ...
          <view 
            v-else 
            class="login-btn" 
            @tap.stop="handleNavigateToLogin"
          >
            去登录
          </view>
        </view>
    ...
    <view class="swiper-container" :style="{ marginTop: `${navbarTotalHeight}px` }">
      ...
      >
        <swiper-item 
          v-for="(item, index) in validBanners" 
...
<script>
import { u_logo, u_bannerlist, u_midMenulist } from '@/api'
import { info_screen } from '@/utils/scheme/screen.js'
...
export default {
  ...
  methods: {
    ...
    async initialBannerList() {
      ...
      try {
        const response = await u_bannerlist({ terminalId: 0 })
    ...
    async initialMenuList() {
      ...
      try {
        const response = await u_midMenulist({ terminalId: -1 })
